...
import { graph } from "../algorithms/dijkstraSeven";
...
const findVertexByObjectId = (vertexId: string) =>
  graphDataSeven.vertices.find((v) => v.objectName === vertexId);

export function navigateToObjectSeven(
  ...
  const pathString = shortestPath
    .slice(1)
    .map((vertexId) => {
      const vertex = graphDataSeven.vertices.find((v) => v.id === vertexId);
      return vertex ? `L${vertex.cx} ${vertex.cy}` : "";
    })
    .join(" ");

  const startVertex = graphDataSeven.vertices.find(
...
export function resetEdges() {
  ...
  graphDataSeven.edges.forEach((edge) => {
...
export function navigateWithDelay(
  ...
  if (index < objects.length) {
    ...
    navigateToObjectSeven(obj.name, navigation, setNavigation);
