import { graphDataSecond, VertexDataSecond } from "@/store/graphDataSecond";
...
graphDataSecond.vertices.forEach((vertex) => {
...
graphDataSecond.edges.forEach((edge) => {
  ...
  const { from, to } = edge;
  const fromVertex = graphDataSecond.vertices.find(
    (vertex) => vertex.id === from
  );
  const toVertex = graphDataSecond.vertices.find((vertex) => vertex.id === to);
...
function calculateDistance(
  vertex1: VertexDataSecond,
  vertex2: VertexDataSecond
) {
